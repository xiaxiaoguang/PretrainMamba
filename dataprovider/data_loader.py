...
class Dataset_KDDcup(Dataset):
    ...
    def data_preprocess(self, df_data):
        ...
        #将Prtv,Patv小于0置0
        # new_df_data.loc[raw_df_data['Prtv']<0,('Prtv')] = -100 ### 原本这三个都是-100
...
class Dataset_Finance(Dataset):
    ...
    def inverse_transform(self, data): # 不会调用，scaler=None
        return self.scaler.inverse_transform(data)
...
class Dataset_ETT_hour(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # seq_x = self.data_x[s_begin:s_end,:,np.newaxis].transpose(2,0,1)
        # seq_y = self.data_y[r_begin:r_end,:,np.newaxis].transpose(2,0,1)
